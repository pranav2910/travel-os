 ...
 * trip_01J...:CREATE-ORDER:1}. The scope may itself be a colon-separated path that names the
 * business context, e.g. {@code TRIP:trip_01J...:DISRUPTION:dsr_01J...:CHANGE:1} (scope = {@code
 * TRIP:trip_01J...:DISRUPTION:dsr_01J...}, command = {@code CHANGE}). Twenty retries of the same
 * key must produce one airline order; the owning table enforces this with {@code
...
public record IdempotencyKey(String value) {
  ...
  private static final Pattern FORMAT =
      Pattern.compile(
          "^((?:[A-Za-z0-9_-]{1,64}:)*[A-Za-z0-9_-]{1,64}):([A-Z][A-Z0-9-]{0,63}):([0-9]{1,9})$");

  public IdempotencyKey {
    Objects.requireNonNull(value, "idempotency key");
    if (value.length() > MAX_LENGTH) {
      throw new IllegalArgumentException(
          "idempotency key longer than " + MAX_LENGTH + " characters: " + value);
    }
