...
## Pilot scope: what this sandbox pilot does and does not do

Stated so the pilot is judged on what exists (the 2026-09 audit tested "conversational agent"
capabilities that were never built; they are listed here as absent, not as untested).

| Exists | Does not exist (not part of the pilot) |
|---|---|
| Structured requests (round trip, one way, multi-city itinerary with stays and transfers) and **single-shot** free text: one message → one extracted intent → the platform plans, seeks approval when policy says so, and books with the **simulated** suppliers. Nothing here is a preview. | A **conversational agent**: no multi-turn dialogue, no "option 2 then option 3", no "change the plan mid-search", no "book it" as a separate step, no plan-only mode. |
| Locations: the airports in the platform catalog (`libs/common … Locations`, ~100 airports on every path, round trips and itineraries alike). A city code (NYC, LON, PAR, TYO …) is refused with the airports it stands for; any other code is refused as unknown (`UNKNOWN_LOCATION`). | Metro-area search, airline/cabin/stop preferences as request fields, several travelers per trip, editing or reordering a submitted trip. |
| Dates: a departure window must still be open when the trip is submitted, judged against the platform clock and reported in the departure airport's local time (`DEPARTURE_IN_PAST`); the workflow checks again right before booking (`DEPARTURE_PASSED`). | A booking horizon or a maximum trip length. |
| Money: a trip is priced in USD, the currency the sandbox airline quotes (`CURRENCY_UNSUPPORTED` otherwise); the platform never converts, so a hotel quoted in another currency (London, GBP) is refused by policy rather than converted. Finance records settled refunds in the order's currency; a refund of zero is legitimate (a non-refundable rate settled at nothing) and is ledgered as such. | Real money, a payment provider, live supplier connections, e-mail. |
| Cancellation: a trip that holds no reservation is cancelled at once. A **booked** trip becomes `CANCELLING`: the Order service releases every component at the suppliers and the trip is `CANCELLED` only once that is established. A supplier that refuses (non-refundable) leaves the trip `CANCELLING` with `CANCELLATION_INCOMPLETE` and an open exposure for a person; the reservation is never shown as cancelled while it is confirmed somewhere. | Partial cancellation of one component, changes to a booked trip other than disruption recovery. |
| Arranged trips: a manager or travel admin may book for another employee and must give that traveler's name and e-mail (`TRAVELER_IDENTITY_REQUIRED`); the reservation is made in the traveler's name. | Traveler profiles, passports, loyalty numbers (an Enterprise Context concern for a later slice). |
