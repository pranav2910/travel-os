...
| Request & planning | Draft requests, search/compare without booking, option selection, price refresh, explicit purchase authorization | absent (submitting books; autonomy implicit in policy ALLOW) | `TripWorkflowImpl` books after APPROVED without a separate authorization |
| Request & planning | Booking horizon / trip-length rules | absent | QA RISK-04 |
| Request & planning | Conversational (multi-turn) request flow | absent | README pilot scope |
...
| Booking | Purchase authorization bound to travelers/itinerary/price/conditions; revalidation before execution | partial (quote revalidation + re-approval on price change exists; no authorization object) | `ItineraryFlow` revalidation |
