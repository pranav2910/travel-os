...
c = qa.api("carol", "POST", "/api/v1/trips", {**qa.round_trip(f"{T} persona: carol arranges for alice", 56, 58), "travelerId": "emp_1001"})
d = qa.api("dan", "POST", "/api/v1/trips", {**qa.round_trip(f"{T} persona: dan tries to arrange for alice", 56, 58), "travelerId": "emp_1001"})
...
qa.record(test_id="P1-PERSONA-01", source_requirement="Prompt 1 personas (power user vs adversarial); checklist 9 travelers (arranging for others)", priority="P1", test_layer="API", account="carol (TRAVEL_ADMIN), dan (TRAVELER)",
          input="POST /trips with travelerId=emp_1001 (alice)", steps="carol then dan", expected="carol 202 and the trip's traveler is alice; dan refused (403) and no trip created",
          actual=f"carol={c.status} traveler={(ct.get('traveler') or {}).get('employeeId')}; dan={d.status} {(d.json or {}).get('code')}", status="PASS" if c.status == 202 and (ct.get("traveler") or {}).get("employeeId") == "emp_1001" and d.status == 403 else "FAIL",
