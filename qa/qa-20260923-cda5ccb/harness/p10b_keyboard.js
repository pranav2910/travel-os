...
(async () => {
  ...
  // ---------------- keyboard-only journey
  {
    ...
    // ---------------- stored HTML purpose (alice's trip from P3-VAL-06) renders as text
  ...
  try { const wb = await webkit.launch(); out.engines.webkit = wb.version(); const page = await (await wb.newContext({ ...devices['Desktop Safari'] })).newPage(); await page.goto(BASE + '/'); await page.getByRole('button', { name: 'Sign in' }).click(); await page.waitForURL(/realms\/travelos/); await page.getByRole('textbox', { name: /username|email/i }).fill('alice'); await page.getByRole('textbox', { name: /^password$/i }).fill(PASSWORD); await page.getByRole('button', { name: /sign in/i }).click(); await page.waitForURL((u) => u.origin === BASE && !u.pathname.startsWith('/auth/')); await page.goto(BASE + '/trips'); out.engines.webkitSmoke = { tripsHeading: await page.getByRole('heading', { name: 'Trips' }).isVisible(), rows: await page.getByRole('row').count() }; await page.screenshot({ path: path.join(EV, 'p10-webkit-trips.png') }); await wb.close(); } catch (e) { out.engines.webkitSmoke = { error: e.message.slice(0, 120) }; }
