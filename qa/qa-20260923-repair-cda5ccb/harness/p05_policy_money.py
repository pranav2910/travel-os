...
for label, amt, cur, expect in [("zero", 0, "USD", (200, 201, 202)), ("one cent", 1, "USD", (200, 201, 202)), ("negative", -100, "USD", (400, 422)), ("huge", 10**15, "USD", (200, 201, 202, 400, 422)), ("wrong currency EUR on a USD order", 100, "EUR", (400, 422))]:
    ...
    qa.record(test_id=f"P5-MONEY-{label.split()[0].upper()}", source_requirement=f"Prompt 5 money: refund {label}", priority="P1", test_layer="API", account="carol (FINANCE)", input=json.dumps(body)[:120], steps="POST /learning/outcomes/refunds",
              expected=f"status in {expect} (contract: zero is a legitimate settlement, negative refused, a currency other than the order's refused with CURRENCY_MISMATCH, never converted)", actual=f"{rr.status} {(rr.json or {}).get('code') if isinstance(rr.json, dict) else ''} {json.dumps(rr.json)[:100] if rr.status < 300 else ''}", status="PASS" if rr.status in expect else "FAIL", evidence=qa.evidence(f"p5-refund-{label.split()[0]}.json", {"request": body, "status": rr.status, "body": rr.json}))
