...
qa.record(test_id="P3-MUT-02", source_requirement="Prompt 3 five rapid clicks / two tabs with one logical request", priority="P0", test_layer="API (5 concurrent threads)", account="alice", input="one key, five simultaneous POSTs",
          steps="ThreadPoolExecutor(5)", expected="exactly one trip; every answer 202/200 with the same id (BUG-07: no 500, no conflict for an identical request); never five trips", actual=f"statuses={[r.status for r in rs]} distinct ids={len(ids)}",
          status="PASS" if len(ids) == 1 and all(r.status in (200, 202) for r in rs) else "FAIL", evidence=qa.evidence("p3-idempotency-concurrent.json", [{"status": r.status, "body": r.json} for r in rs]), ids=" ".join(sorted(i for i in ids if i)))
...
first_status = (c1.json or {}).get("status")
...
qa.record(test_id="P3-LIFE-01", source_requirement="Prompt 3 delete/cancel with confirmed bookings per the documented lifecycle; repeat", priority="P0", test_layer="API", account="alice", input=f"BOOKED trip {t0}",
          steps="POST cancellation twice", expected="BOOKED -> CANCELLING (200) -> CANCELLED only once the order is CANCELLED at the supplier; a second cancellation is a no-op (200), never a second side effect",
          actual=f"first {c1.status} -> {first_status}; final={st}; second {c2.status} {code(c2) or ''}; order status={[x.get('status') for x in o]}", status="PASS" if c1.status == 200 and first_status in ("CANCELLING", "CANCELLED") and st == "CANCELLED" and o and all(x.get("status") == "CANCELLED" for x in o) and c2.status == 200 else "FAIL", evidence=qa.evidence("p3-cancel.json", {"first": c1.json, "second": {"status": c2.status, "body": c2.json}, "orders": o}), ids=t0)
