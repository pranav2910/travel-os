...
(async () => {
  ...
  // ---------------- accessibility scans on every page as an admin (carol sees everything)
  {
    ...
    if (await link.count()) { const href = await link.getAttribute('href'); await page.goto(BASE + href); await page.waitForURL(/\/trips\/trip_/, { timeout: 20000 }); await page.waitForTimeout(2000); const res = await new AxeBuilder({ page }).withTags(['wcag2a', 'wcag2aa']).analyze(); out.a11y['/trips/:id'] = { violations: res.violations.map((v) => ({ id: v.id, impact: v.impact, nodes: v.nodes.length, help: v.help })) }; }
  ...
  // ---------------- keyboard-only journey: Tab/Enter from the sign-in button through a submitted trip
  {
    ...
    await page.getByRole('button', { name: 'Sign in' }).waitFor(); // the audit's p10b fix: tab only once the button exists
    ...
    // the keyboard journey proper (tab stops, focus ring, dialog) is p10b's; here the login only has to proceed
    await page.getByRole('button', { name: 'Sign in' }).focus(); await page.keyboard.press('Enter'); await page.waitForURL(/realms\/travelos/, { timeout: 20000 });
    ...
    // dates are filled (as p10b does): typing digits into a date input is not reliable across Chromium builds
    await page.getByRole('group', { name: 'Outbound' }).getByLabel('Date').fill(future(100));
    await page.getByRole('group', { name: 'Return' }).getByLabel('Date').fill(future(102));
