...
def done(tid, deadline=220):
    ...
    if st == "AWAITING_APPROVAL":
        # an intercontinental itinerary crosses the seed policy's approval line: bob decides, as a
        # manager would, so the style is judged on its booking and not on the policy stop
        qa.api("bob", "POST", f"/api/v1/trips/{tid}/approval", {"decision": "APPROVE", "comment": f"{T} style approved for the test"})
        st = qa.wait_trip("alice", tid, {"BOOKED", "FAILED"}, deadline)
...
for sid, label, why in [
    ...
    ("P6-S08", "JFK only vs any NYC airport", "the request takes one airport code per leg; there is no metro search. A city code (NYC) is refused with the airports it stands for (UNKNOWN_LOCATION: JFK, EWR, LGA) — BUG-01 fixed; the search over several airports stays out of scope"),
...
import os
if os.environ.get("P6_ONLY"):
    cases = [c for c in cases if c[0] in set(os.environ["P6_ONLY"].split(","))]
